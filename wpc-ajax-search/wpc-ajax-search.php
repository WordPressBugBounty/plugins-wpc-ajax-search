        ...
        if ( ! class_exists( 'WPCleverWpcas' ) && class_exists( 'WC_Product' ) ) {
            class WPCleverWpcas {
                ...
                function __construct() {
                    ...
                    if ( ( self::get_setting( 'search_sku', 'no' ) === 'yes' ) && ( (array) self::get_setting( 'search_post_types', [ 'product' ] ) === [ 'product' ] ) ) {
                        add_filter( 'pre_get_posts', [ $this, 'search_sku' ], 99 );
                    }

                    if ( ( self::get_setting( 'search_tag', 'no' ) === 'yes' ) && ( (array) self::get_setting( 'search_post_types', [ 'product' ] ) === [ 'product' ] ) ) {
                        add_filter( 'pre_get_posts', [ $this, 'search_tag' ], 99 );
                    }

                    // frontend ajax
                    add_action( 'wc_ajax_wpcas_load', [ $this, 'ajax_load' ] );
                    ...
                    // nonce check
                    add_filter( 'wpcas_disable_nonce_check', function ( $check, $context ) {
                        return apply_filters( 'wpcas_disable_security_check', $check, $context );
                    }, 10, 2 );
                }
                ...
                function enqueue_scripts() {
                    ...
                    wp_localize_script( 'wpcas-frontend', 'wpcas_vars', [
                                    ...
                                    'ajax_load'            => apply_filters( 'wpcas_ajax_load', false ),
                    ...
                    <div class="wpclever_settings_page wrap">
                        ...
                        <div class="wpclever_settings_page_content">
                            <?php if ( $active_tab === 'settings' ) {
                                ...
                                $search_tag            = self::get_setting( 'search_tag', 'no' );
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div style="margin-bottom: 10px;">
                                                    <input type="text" name="wpcas_settings[keyword_recent]"
                                                           value="<?php echo esc_attr( self::get_setting( 'keyword_recent', '' ) ); ?>"/>
                                                    <span class="description"><?php esc_html_e( 'Keyword for recent products.', 'wpc-ajax-search' ); ?></span>
                                                </div>
                                                <div style="margin-bottom: 10px;">
                                                    <input type="text" name="wpcas_settings[keyword_featured]"
                                                           value="<?php echo esc_attr( self::get_setting( 'keyword_featured', '' ) ); ?>"/>
                                                    <span class="description"><?php esc_html_e( 'Keyword for featured products.', 'wpc-ajax-search' ); ?></span>
                                                </div>
                                                <div>
                                                    <input type="text" name="wpcas_settings[keyword_popular]"
                                            ...
                                            <td>
                                                ...
                                                if ( ! empty( $post_types ) ) {
                                                    foreach ( $post_types as $post_type ) {
                                                        ?>
                                                        <input type="checkbox"
                                                               name="wpcas_settings[search_post_types][]"
                                                               value="<?php echo esc_attr( $post_type->name ); ?>" <?php checked( in_array( $post_type->name, $search_post_types ) ); ?>/>
                                                        <label><?php echo esc_html( $post_type->label ); ?></label><br/>
                                                        <?php
                                                    }
                                                }
                                                ?>
                ...
                function ajax_load() {
                    if ( ! apply_filters( 'wpcas_disable_nonce_check', false, 'load' ) ) {
                        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'wpcas-security' ) ) {
                            die( '<div id="wpcas-area">Permissions check failed.</div>' );
                        }
                    }
                ...
                function ajax_search() {
                    if ( ! apply_filters( 'wpcas_disable_nonce_check', false, 'search' ) ) {
                        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'wpcas-security' ) ) {
                    ...
                    if ( ! $products ) {
                        ...
                        $more_results = ( $post_types === [ 'product' ] ) && ( self::get_setting( 'more_results', 'yes' ) === 'yes' ) && ( $query->found_posts > $limit ) ? $query->found_posts : 0;
                    }
                ...
                function show_products( $products, $keyword = '', $category = 0, $more_results = 0 ) {
                    ...
                    foreach ( $products as $pid ) {
                        if ( $product = wc_get_product( $pid ) ) {
                            // product
                            if ( ( self::get_setting( 'exclude_unpurchasable', 'no' ) === 'yes' ) && ( ! $product->is_in_stock() || ! $product->is_purchasable() ) ) {
                                continue;
                            }

                            if ( ( self::get_setting( 'exclude_hidden', 'no' ) === 'yes' ) && ( $product->get_catalog_visibility() !== 'visible' ) && ( $product->get_catalog_visibility() !== 'search' ) ) {
                                continue;
                            }

                            if ( apply_filters( 'wpcas_exclude_product', false, $product ) ) {
                                continue;
                            }

                            echo '<div class="wpcas-product">';
                            echo '<div class="wpcas-product-inner">';
                            do_action( 'wpcas_before_product_inner', $product );
                            ...
                            do_action( 'wpcas_before_product_info', $product );
                            ...
                            do_action( 'wpcas_product_actions', $product );
                            echo '</div><!-- /wpcas-product-actions -->';

                            do_action( 'wpcas_after_product_inner', $product );
                            echo '</div><!-- /wpcas-product-inner -->';
                            echo '</div><!-- /wpcas-product -->';
                        } else {
                            // other post types
                            echo '<div class="wpcas-product wpcas-post">';
                            echo '<div class="wpcas-product-inner wpcas-post-inner">';
                            do_action( 'wpcas_before_post_inner', $pid );

                            echo '<div class="wpcas-product-thumb wpcas-post-thumb">';
                            do_action( 'wpcas_before_post_thumb', $pid );
                            ...
                            do_action( 'wpcas_after_post_thumb', $pid );
                            echo '</div><!-- /wpcas-post-thumb -->';
                            echo '<div class="wpcas-product-info wpcas-post-info">';
                            do_action( 'wpcas_before_post_info', $pid );

                            echo '<div class="wpcas-product-name wpcas-post-name">';
                            ...
                            do_action( 'wpcas_after_post_info', $pid );
                            echo '</div><!-- /wpcas-post-info -->';

                            do_action( 'wpcas_after_post_inner', $pid );
                ...
                function search_tag( $query ) {
                    if ( $query->is_search && isset( $query->query['is_wpcas'] ) ) {
                        ...
                        JOIN {$wpdb->prefix}term_taxonomy AS tt ON ( tr.term_taxonomy_id = tt.term_taxonomy_id AND tt.taxonomy = 'product_tag' )
                ...
                function get_search_area() {
                    ...
                    $area_class = apply_filters( 'wpcas_area_class', 'wpcas-area wpcas-position-' . self::get_setting( 'position', '01' ) . ' wpcas-slide-' . self::get_setting( 'effect', 'yes' ) );
                ...
                function footer() {
                    ...
                    if ( apply_filters( 'wpcas_ajax_load', false ) ) {
                        echo '<div id="wpcas-area"></div>';
                    } else {
                        self::get_search_area();
                    }
                    ...
                    <div class="wpcas_rule <?php echo esc_attr( $active ? 'active' : '' ); ?>"
                        ...
                        <div class="wpcas_rule_content">
                            <div class="wpcas_tr">
                                ...
                                <div class="wpcas_td">
                                    <input type="text" class="regular-text wpcas_rule_name_val"
                                           name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][name]"
                    ...
                    <div class="wpcas_tr">
                        ...
                        <div class="wpcas_td wpcas_rule_td">
                            ...
                            <div class="wpcas_add_condition">
                                <a class="wpcas_new_condition"
                                   href="#"><?php esc_attr_e( '+ Add condition', 'wpc-ajax-search' ); ?></a>
                            </div>
                        </div>
                    </div>
                    <?php
                ...
                function condition( $condition_key = '', $condition = [], $rule_key = '' ) {
                    ...
                    <div class="wpcas_condition">
						<span class="wpcas_condition_compare">
							<select name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][conditions][<?php echo esc_attr( $condition_key ); ?>][compare]"
                                    class="wpcas_condition_compare_selector">
							...
							<select name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][conditions][<?php echo esc_attr( $condition_key ); ?>][text][]"
                                    class="wpcas_condition_text_val" multiple>
								<?php
                ...
                function returned( $rule_key = '', $rule = [] ) {
                    ...
                    <div class="wpcas_tr">
                        ...
                        <div class="wpcas_td wpcas_rule_td">
                            <select name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][returned]"
                                    class="wpcas_returned_selector">
                    ...
                    <div class="hide_returned show_if_returned_products">
                        <div class="wpcas_tr">
                            ...
                            <div class="wpcas_td wpcas_rule_td">
                                <select class="wpcas_source_selector"
                                        name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][get]">
                                    ...
                                    ?>
										...
										<span><?php esc_html_e( 'Limit', 'wpc-ajax-search' ); ?> <input type="number"
                                                                                                        min="1" max="50"
                                                                                                        name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][limit]"
                                                                                                        value="<?php echo esc_attr( $limit ); ?>"/></span>
										<span>
										<?php esc_html_e( 'Order by', 'wpc-ajax-search' ); ?> <select
                                                    name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][orderby]">
										...
										<span><?php esc_html_e( 'Order', 'wpc-ajax-search' ); ?> <select
                                                    name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][order]">
                        ...
                        <div class="wpcas_tr hide_get show_if_products">
                            ...
                            <div class="wpcas_td wpcas_rule_td">
                                <select class="wc-product-search wpcas-product-search"
                                        name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][products][]"
                                        multiple="multiple"
                                        data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'wpc-ajax-search' ); ?>"
                        ...
                        <div class="wpcas_tr show_get hide_if_all hide_if_products hide_if_combined">
                            ...
                            <div class="wpcas_td wpcas_rule_td">
                                <select class="wpcas_terms"
                                        name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][terms][]"
                                        multiple="multiple"
                                        data-<?php echo esc_attr( $get ); ?>="<?php echo esc_attr( implode( ',', $terms ) ); ?>">
                                    <?php
                                    if ( ! empty( $terms ) ) {
                                        foreach ( $terms as $at ) {
                ...
                function combined( $combined_key = '', $combined = [], $rule_key = '' ) {
                    ...
                    <div class="wpcas_combined">
                        <span class="wpcas_combined_remove">&times;</span> <span class="wpcas_combined_selector_wrap">
                                    <select class="wpcas_combined_selector"
                                            name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][combined][<?php echo esc_attr( $combined_key ); ?>][apply]">
	                                    <?php
							...
							<select class="wpcas_combined_compare"
                                    name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][combined][<?php echo esc_attr( $combined_key ); ?>][compare]">
								...
								<option value="is_not" <?php selected( $compare, 'is_not' ); ?>><?php esc_html_e( 'excluding', 'wpc-ajax-search' ); ?></option>
                                    ...
                                    <select class="wpcas_combined_val wpcas_apply_terms" multiple="multiple"
                                            name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][combined][<?php echo esc_attr( $combined_key ); ?>][terms][]">
                                        <?php
                                        if ( ! empty( $terms ) ) {
                                            foreach ( $terms as $ct ) {
							...
							<select class="wpcas_combined_number_compare"
                                    name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][combined][<?php echo esc_attr( $combined_key ); ?>][number_compare]">
								...
								<option value="less_equal" <?php selected( $number_compare, 'less_equal' ); ?>><?php esc_html_e( 'less or equal', 'wpc-ajax-search' ); ?></option>
                                    ...
                                    <input type="number" class="wpcas_combined_number_val"
                                           value="<?php echo esc_attr( $number_value ); ?>"
                                           name="wpcas_rules[<?php echo esc_attr( $rule_key ); ?>][combined][<?php echo esc_attr( $combined_key ); ?>][number_value]"/>
                                </span>
                    </div>
                    <?php
                }
                ...
                function shortcode_search_form() {
                    ...
                    echo '<div class="wpcas-search-form">';
